...
import UsuarioItem from './UsuarioItemEm'
...
function UserList(props) {
    ...
    const loadData = () => {
        setIsLoading(true);
        axios.get('/api/v1/usuario/listarEmp?page='+state.currentPage, {
            params: {
        ...
        confirmAlert({
            ...
            buttons: [{
                ...
                onClick: () => {
                    ...
                    axios.post('/api/v1/usuario/block', {
                        api_token: authUser.api_token,
                        id: id
                    })
                    .then(response => {
        ...
        confirmAlert({
            ...
            buttons: [{
                ...
                onClick: () => {
                    setIsLoading(true);
                    //ruta de la api en webapi    
                    axios.post('/api/v1/usuario/unblock', {
                        api_token: authUser.api_token,
                        id: id
                    })
                    .then(response => {
    ...
    const dataTable = () => {
        ...
        (users.length == 0 ? <tr className="text-center text-gray">
                                <td className="p-3 font-weight-bold"><h1> No hay datos</h1></td>
                            </tr> : 
        users.map((user, i) => {
            return <UsuarioItem onClickDeleteHandler={onClickDeleteHandler} onClickBlockUser={onClickBlockUser} onClickUnblockUser={onClickUnblockUser} obj={user} key={i} />;
        })
    ...
    const exportExcelFile = () => {
        ...
        const sheet = workbook.addWorksheet("Usuarios_Emprendedores");
       ...
       users.map(users => {
        ...
        workbook.xlsx.writeBuffer().then(users =>{
            ...
            anchor.href = url;
            anchor.download = "Emprendedores.xlsx";
            anchor.click();
