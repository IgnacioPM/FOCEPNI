...
import TopControlImage from "./TopControlImages";
...
const ImageList = (props) => {
    ...
    useEffect(() => {
        document.title = "Todas las imagenes";

        props.setActiveComponentProp("ListarImagenes");
    }, []);
    ...
    const loadData = () => {
        setIsLoading(true);
        axios
            .get("/api/v1/galeria/listar?page=" + state.currentPage, {
                params: {
            ...
            .then((response) => {
                setIsLoading(false);
                setImages(response.data.message.data);
                setState({
                    ...state,
                    ...
                    imagesArray: response.data.message.data,
                });
            })
            .catch((error) => {
                showSznNotification({
    ...
    const onClickDeleteHandler = (id) => {
        confirmAlert({
            title: "Desea desabilitar la imagen de la galería",
            message: "Está seguro?",
            buttons: [
                {
                    label: "Si",
    ...
    const onClickActiveHandler = (id) => {
        confirmAlert({
            title: "Desea habilitar la imagen de la galería",
            message: "Está seguro?",
            buttons: [
                {
                    label: "Si",
    ...
    const print = () => {
        ...
        doc.autoTable(columns, state.imagesArray, {
            theme: "grid",
        ...
        doc.save("Reporte_Imagenes.pdf");
    };

    const exportExcelFile = () => {
        const workbook = new ExcelJS.Workbook();
        const sheet = workbook.addWorksheet("Reporte_Imagenes");
        ...
        workbook.xlsx.writeBuffer().then(img =>{
            ...
            anchor.href = url;
            anchor.download = "Reporte_Imagenes.xlsx";
            anchor.click();
