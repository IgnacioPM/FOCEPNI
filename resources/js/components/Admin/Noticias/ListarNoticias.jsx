...
import TopControlNews from "./TopControlNews";
...
const ListarNoticia = (props) => {
    ...
    useEffect(() => {
        document.title = "Todas las Noticias";

        props.setActiveComponentProp("ListarNoticias");
    }, []);
    ...
    const loadData = () => {
        setIsLoading(true);
        axios
            .get("/api/v1/noticias/listar?page=" + state.currentPage, {
                params: {
            ...
            .then((response) => {
                setIsLoading(false);
                setNews(response.data.message.data);
                setState({
                    ...state,
    ...
    const onClickDeleteHandler = (id) => {
        confirmAlert({
            title: "Desea desabilitar la noticia",
            message: "Está seguro?",
            buttons: [
                {
                    label: "Si",
    ...
    const print = () => {
        ...
        doc.autoTable(columns, state.newsArray, {
            theme: "grid",
    ...
    const exportExcelFile = () => {
        ...
        const sheet = workbook.addWorksheet("Reporte_Noticias");
        ...
        workbook.xlsx.writeBuffer().then(doc =>{
            ...
            anchor.href = url;
            anchor.download = "Reporte_Noticias.xlsx";
            anchor.click();
