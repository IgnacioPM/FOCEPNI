...
import SolicitudItem from "./ProductItemAdmin";
import TopControl from "./TopControlProductsAdmin";
import jsPDF from "jspdf";
const ExcelJS = require('exceljs');

const verSolicitudesProducts = (props) => {
    ...
    useEffect(() => {
        document.title = "Todas las Solicitudes";

        props.setActiveComponentProp("ListProductsAdmin");
    }, []);
    ...
    const loadData = () => {
        setIsLoading(true);
        axios
            .get("/api/v1/productsEm/listar?page=" + state.currentPage, {
                params: {
            ...
            .then((response) => {
                setIsLoading(false);
                setsProduct(response.data.message.data);
                setState({
                    ...state,
                    ...
                    arraySoliP: response.data.message.data,
                });
            })
            .catch((error) => {
                showSznNotification({
    ...
    const print = () => {
        var columns = [
            { title: "Nombre Emprendimiento", dataKey: "" },
            { title: "Producto", dataKey: "nombreP" },
            { title: "Categoria", dataKey: "categoriaP" },
            { title: "Imagen", dataKey: "imagenP" },
        ];
        ...
        doc.setTextColor("#000");nombreEmprendimiento
        ...
        doc.text(" Reporte de solicitudes productos", 50, 68);
        ...
        doc.autoTable(columns, state.arraySoliP, {
            theme: "grid",
        ...
        doc.save("Reporte_solicitudes_Productos.pdf");
    };

    const exportExcelFile = () => {
        const workbook = new ExcelJS.Workbook();
        const sheet = workbook.addWorksheet("Solicitud_Producto");
        ...
        sProduct.map(prod => {
        sheet.addRow({
            nEmp: prod.nombreEmprendimiento,
            nProduct: prod.nombreP,
            CatP: prod.categoriaP,
            ImP: prod.imagenP,
        ...
        workbook.xlsx.writeBuffer().then(prod =>{
            const blob = new Blob( [prod], {
            type: "application/vnd.openxmlformats-officedocument.spreadsheet.sheet",
    });
            const url = window.URL.createObjectURL(blob);
            const anchor = document.createElement('a');
            anchor.href = url;
            anchor.download = "Solicitud_Productos.xlsx";
            anchor.click();
